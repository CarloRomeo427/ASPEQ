### AGENT CODE FOR FASPEQ PLOSS TRAIN (Policy Loss on Training Set)
### Filename: agent_ploss_train.py
...
from src.algos.core import TanhGaussianPolicy, Mlp, soft_update_model1_with_model2, ReplayBuffer, \
    mbpo_target_entropy_dict, soft_update_policy, test_agent
import wandb
...
class EffectiveRankMonitor:
    ...
    def compute_condition_number(singular_values: torch.Tensor, eps: float = 1e-10) -> float:
        ...
        max_sv = singular_values[0]; min_sv = singular_values[-1]
        if min_sv < eps: return float('inf')
        return (max_sv / min_sv).item()
    
    @staticmethod
    def compute_gradient_metrics(gradients: torch.Tensor) -> dict:
        if gradients.dim() == 1: gradients = gradients.unsqueeze(1)
        try: U, S, Vh = torch.linalg.svd(gradients, full_matrices=False)
        except: return {'effective_rank': 0.0, 'condition_number': float('inf'), 'spectral_norm': 0.0, 'stable_rank': 0.0}
        ...
        return {'effective_rank': effective_rank, 'condition_number': condition_number, 'spectral_norm': spectral_norm, 'stable_rank': stable_rank}
...
def get_probabilistic_num_min(num_mins):
    ...
    if num_mins - floored_num_mins > 0.001:
        ...
        if np.random.uniform(0, 1) < prob_for_higher_value: return int(floored_num_mins + 1)
        else: return int(floored_num_mins)
    else: return num_mins
...
class Agent(object):
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
        ...
        if auto_alpha:
            if target_entropy == 'auto': self.target_entropy = - act_dim
            if target_entropy == 'mbpo':
                mbpo_target_entropy_dict['AntTruncatedObs-v2'] = -4
                mbpo_target_entropy_dict['HumanoidTruncatedObs-v2'] = -2
                try: self.target_entropy = mbpo_target_entropy_dict[env_name]
                except: self.target_entropy = -2
        ...
        if auto_stab: self.next_trigger_size = int(start_steps * adaptive_trigger_expansion_rate)
        else: self.next_trigger_size = 10000
        self.auto_stab = auto_stab
        
        self.num_reference_batches = num_reference_batches
        self.effective_rank_monitor = EffectiveRankMonitor()

    def __get_current_num_data(self): return self.replay_buffer.size
    ...
    def store_data(self, o, a, r, o2, d):
        ...
        # Note: Validation buffer NOT used for stabilizing here, but kept for compatibility
    ...
    def populate_val_buffer_from_offline(self):
        if self.replay_buffer_offline.size == 0: return
        ...
        indices = np.random.choice(self.replay_buffer_offline.size, size=min(num_samples, self.replay_buffer_offline.size), replace=False)
        for idx in indices:
            self.val_replay_buffer.store(all_data['obs1'][idx], all_data['acts'][idx], all_data['rews'][idx], all_data['obs2'][idx], all_data['done'][idx])
    ...
    def check_should_trigger_offline_stabilization(self):
        ...
        if self.auto_stab:
            if current_size >= self.next_trigger_size:
                ...
                print(f"\n{'='*80}\nOFFLINE STABILIZATION TRIGGERED (Adaptive PLoss-Train)\n{'='*80}")
                return True
        else:
            if current_size >= self.next_trigger_size:
                self.last_trigger_buffer_size = current_size
                self.next_trigger_size = self.last_trigger_buffer_size + 10000
                print(f"\n{'='*80}\nOFFLINE STABILIZATION TRIGGERED (Fixed PLoss-Train)\n{'='*80}")
                return True
        return False
    ...
    def get_redq_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor):
        with torch.no_grad():
            ...
            sample_idxs = None
            if self.q_target_mode == 'min': q_target = torch.min(q_prediction_next_cat, 1, keepdim=True)[0]
            elif self.q_target_mode == 'ave': q_target = torch.mean(q_prediction_next_cat, 1, keepdim=True)
    ...
    def compute_effective_rank_metrics(self, batch_size=256):
        ...
        for q_idx, q_net in enumerate(self.q_net_list):
            ...
            q_net.zero_grad()
            inputs = torch.cat([obs_tensor, acts_tensor], 1); inputs.requires_grad = True
            ...
            for i in range(batch_size):
                ...
                if inputs.grad is not None: gradients.append(inputs.grad[i].flatten())
            if gradients:
                gradient_matrix = torch.stack(gradients).T
                rank_metrics = self.effective_rank_monitor.compute_gradient_metrics(gradient_matrix)
                for key, value in rank_metrics.items(): metrics[f'q{q_idx}_{key}'] = value
        return metrics
    ...
    def train(self, logger, current_env_step=None):
        ...
        for i_update in range(num_update):
            if self.o2o: obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor = self.sample_data_mix(self.batch_size)
            else: obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor = self.sample_data(self.batch_size)
            ...
            for q_i in range(self.num_Q): self.q_optimizer_list[q_i].zero_grad()
            q_loss_all.backward()
            ...
            if ((i_update + 1) % self.policy_update_delay == 0) or i_update == num_update - 1:
                ...
                for sample_idx in range(self.num_Q): self.q_net_list[sample_idx].requires_grad_(True)

                if self.auto_alpha:
                    alpha_loss = -(self.log_alpha * (log_prob_a_tilda + self.target_entropy).detach()).mean()
                    self.alpha_optim.zero_grad()
                    alpha_loss.backward(); self.alpha_optim.step()
                    self.alpha = self.log_alpha.cpu().exp().item()
                else: alpha_loss = Tensor([0])

            for q_i in range(self.num_Q): self.q_optimizer_list[q_i].step()
            ...
            if i_update == num_update - 1:
                ...
                if current_env_step is not None:
                    wandb.log({"policy_loss": policy_loss.cpu().item(), "mean_loss_q": q_loss_all.cpu().item() / self.num_Q}, step=current_env_step)

        if num_update == 0: logger.store(LossPi=0, LossQ1=0, LossAlpha=0, Q1Vals=0, Alpha=0, LogPi=0, PreTanh=0)
    ...
    def evaluate_training_policy_loss(self, num_batches=10, batch_size=256):
        """Evaluate Policy Loss on randomly sampled Training Data."""
        if self.replay_buffer.size == 0: return 0.0
        total_policy_loss = 0.0
        
        with torch.no_grad():
            for _ in range(num_batches):
                if self.o2o: obs_tensor, _, _, _, _ = self.sample_data_mix(batch_size)
                else: obs_tensor, _, _, _, _ = self.sample_data(batch_size)
    ...
    def finetune_offline_ploss_train(self, epochs, test_env=None, current_env_step=None):
        """ 
        FASPEQ (PLoss-Train Variant): Finetune using Policy Loss on Training Set for early stopping.
        Automatic dynamic length enabled by training loss check.
        """
        print(f"\n{'='*80}\nSTARTING PLOSS-TRAIN OFFLINE STABILIZATION\n{'='*80}")
        print(f"Max epochs: {epochs}")
        print(f"Monitoring: Policy Loss on Train Set (Fixed Policy)")
        
        initial_loss = self.evaluate_training_policy_loss()
        best_loss = initial_loss
        steps_without_improvement = 0
        
        if current_env_step is not None:
            wandb.log({
                "Train_PLoss_Proxy": initial_loss,
                "offline_training": 1
            }, step=current_env_step)
        
        print(f"Initial Train Policy Loss: {initial_loss:.6f}\n")
        
        epochs_performed = 0
        for e in range(epochs):
            epochs_performed = e
            
            # Validation Check (Using Training PLoss as Proxy)
            if e > 0 and e % self.val_check_interval == 0:
                curr_loss = self.evaluate_training_policy_loss()
                
                if current_env_step is not None:
                    wandb.log({
                        "Train_PLoss_Proxy": curr_loss,
                        "offline_epoch": e,
                        "offline_training": 1
                    }, step=current_env_step)
                
                # Improvement = Decrease in Policy Loss (usually)
                if curr_loss < best_loss * 0.999:
                    best_loss = curr_loss
                    steps_without_improvement = 0
                    print(f"Epoch {e:6d}: Train PLoss improved to {curr_loss:.6f}")
                else:
                    steps_without_improvement += self.val_check_interval
                    print(f"Epoch {e:6d}: Train PLoss {curr_loss:.6f} (No improve {steps_without_improvement})")
            ...
            if test_env and (e + 1) % 5000 == 0:
                ...
                if current_env_step is not None:
                    wandb.log({"OfflineEvalReward": np.mean(test_rw), "offline_training": 1}, step=current_env_step)
            
            if self.o2o: obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor = self.sample_data_mix(self.batch_size)
            else: obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor = self.sample_data(self.batch_size)
            
            """Q loss with expectile loss (Q-updates only, Policy Fixed)"""
            y_q, sample_idxs = self.get_redq_q_target_no_grad(obs_next_tensor, rews_tensor, done_tensor)
            q_prediction_list = []
            for q_i in range(self.num_Q):
                q_prediction = self.q_net_list[q_i](torch.cat([obs_tensor, acts_tensor], 1))
        ...
        if current_env_step is not None:
            wandb.log({"offline_training": 0}, step=current_env_step)
