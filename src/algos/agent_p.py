...
class Agent(object):
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
        ...
        if auto_stab:
            # APASPEQ: Adaptive triggering based on expansion rate
            self.next_trigger_size = int(start_steps * adaptive_trigger_expansion_rate)
        else:
            # PASPEQ: Fixed 10k intervals
    ...
    def check_should_trigger_offline_stabilization(self):
        ...
        Two modes:
        1. PASPEQ (auto_stab=False): Fixed 10k intervals (10k, 20k, 30k...)
        2. APASPEQ (auto_stab=True): Adaptive based on validation buffer expansion
        ...
        # APASPEQ: Require validation buffer and adaptive triggering
        if self.auto_stab:
            if self.val_replay_buffer.size < 1000:
                return False
            # Adaptive triggering based on buffer expansion
            if current_size >= self.next_trigger_size:
                ...
                print(f"\n{'='*80}")
                print(f"OFFLINE STABILIZATION TRIGGERED (APASPEQ - Adaptive)")
            ...
            if current_size >= self.next_trigger_size:
                ...
                print(f"\n{'='*80}")
                print(f"OFFLINE STABILIZATION TRIGGERED (PASPEQ - Fixed Interval)")
                print(f"Current buffer size: {current_size}")
                print(f"Next trigger at: {self.next_trigger_size}")
                print(f"{'='*80}\n")
                return True
    ...
    def evaluate_policy_loss(self, num_batches=10, batch_size=256):
        ...
        For PASPEQ (no validation set): sample multiple batches from training buffer.
        
        Args:
            num_batches: Number of batches to sample and average over
        ...
        with torch.no_grad():
            for _ in range(num_batches):
                # Sample batch from main training buffer
                if self.o2o:
    ...
    def evaluate_policy_loss_on_validation(self, batch_size=256):
        ...
        For APASPEQ (with validation set): evaluate on all validation data.
        
        Args:
            batch_size: Batch size for processing validation data
            
        Returns:
            Average policy loss across entire validation set
        """
        if self.val_replay_buffer.size == 0:
            return 0.0
        ...
        with torch.no_grad():
            for i in range(num_batches):
                ...
                # Policy loss: α*log_prob - Q(s, π(s))
                # Sum over batch, then divide by total samples later
    ...
    def finetune_offline_policy(self, epochs, test_env=None, current_env_step=None):
        """ 
        PASPEQ: Finetune with policy loss monitoring (no validation set, fixed intervals).
        
        During offline training, monitor policy loss with FIXED policy.
        If policy loss decreases → continue training
        If policy loss stops improving → early stop
    ...
    def finetune_offline_policy_auto(self, epochs, test_env=None, current_env_step=None):
        ...
        During offline training, monitor policy loss on validation set with FIXED policy.
        If policy loss decreases → continue training
        If policy loss stops improving → early stop
        ...
        for e in range(epochs):
            # Print validation policy loss every 100 epochs for monitoring
            if e > 0 and e % 100 == 0:
            ...
            # Check validation policy loss at regular intervals for early stopping
            if e > 0 and e % self.val_check_interval == 0:
                ...
                # Early stopping logic based on validation policy loss
                improvement = (best_policy_loss - policy_loss) / best_policy_loss * 100 if best_policy_loss > 0 else 0
                if policy_loss < best_policy_loss * 0.999:  # 0.1% improvement threshold
                    best_policy_loss = policy_loss
                    steps_without_improvement = 0
