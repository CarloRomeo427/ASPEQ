### AGENT CODE FOR Cal-QL (Calibrated Q-Learning) ALGORITHM
...
class CalQLReplayBuffer:
    """
    Replay buffer that also stores Monte-Carlo returns for Cal-QL calibration.
    """
    def __init__(self, obs_dim, act_dim, size):
        ...
        self.mc_returns_buf = np.zeros(size, dtype=np.float32)  # Monte-Carlo returns for calibration
    ...
    def sample_batch(self, batch_size):
        ...
        return dict(
            ...
            mc_returns=self.mc_returns_buf[idxs]
        )
    ...
    def sample_all(self):
        return dict(
            ...
            mc_returns=self.mc_returns_buf[:self.size]
        )
...
class Agent(object):
    ...
    This prevents Q-values from being pushed below the behavior policy's value,
    avoiding the "unlearning" phenomenon during online fine-tuning.
    ...
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
        ...
        Cal-QL specific parameters:
            ...
            cql_target_action_gap: Target action gap for Lagrange version (default: 0.8)
        ...
        # Cal-QL / CQL specific hyperparameters
        ...
        # Replay buffers - use CalQL buffer for offline (with MC returns)
        self.replay_buffer = ReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_size)
        self.replay_buffer_offline = CalQLReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_size)
    ...
    def store_data_offline(self, o, a, r, o2, d, mc_return=0.0):
        """Store offline data with Monte-Carlo return for Cal-QL calibration."""
        self.replay_buffer_offline.store(o, a, r, o2, d, mc_return)

    def check_should_trigger_offline_stabilization(self):
        """Cal-QL doesn't use offline stabilization, always returns False."""
        return False
    ...
    def sample_data(self, batch_size):
        ...
        # Online data doesn't have MC returns, use zeros
        mc_returns_tensor = torch.zeros_like(rews_tensor).to(self.device)
        return obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor, mc_returns_tensor
    ...
    def sample_data_mix(self, batch_size):
        ...
        # Online data doesn't have MC returns
        online_mc_returns = np.zeros(online_batch_size, dtype=np.float32)
        ...
        batch = {
            ...
            'mc_returns': np.concatenate([batch_offline['mc_returns'], online_mc_returns], axis=0),
        }
        ...
        rews_tensor = Tensor(batch['rews']).unsqueeze(1).to(self.device)
        done_tensor = Tensor(batch['done']).unsqueeze(1).to(self.device)
        mc_returns_tensor = Tensor(batch['mc_returns']).unsqueeze(1).to(self.device)
        return obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor, mc_returns_tensor
    ...
    def compute_calql_q_loss(self, obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, 
                              done_tensor, mc_returns_tensor, use_calibration=True):
        """
        Cal-QL Q Loss: CQL loss with calibration against Monte-Carlo returns.
        
        The key modification from CQL is:
        - CQL: logsumexp(Q(s, a_sampled)) - Q(s, a_data)
        - Cal-QL: logsumexp(max(Q(s, a_sampled), mc_return)) - Q(s, a_data)
        
        This calibration prevents Q-values from being pushed too low.
        """
        ...
        # ═══════════════════════════════════════════════════════════════════
        # 3. CQL CONSERVATIVE REGULARIZER WITH CAL-QL CALIBRATION
        # ═══════════════════════════════════════════════════════════════════
        ...
        for q_i in range(self.num_Q):
            ...
            # ═══════════════════════════════════════════════════════════════
            # CAL-QL MODIFICATION: Calibrate Q-values against MC returns
            # ═══════════════════════════════════════════════════════════════
            if use_calibration:
                # mc_returns_tensor: [batch, 1] -> expand to [n_actions, batch, 1]
                mc_returns_expanded = mc_returns_tensor.unsqueeze(0).expand(self.cql_n_actions, -1, -1)
                
                # Apply calibration: max(Q, mc_return)
                # This prevents Q-values from being pushed below the behavior policy's value
                q_random_calibrated = torch.max(q_random, mc_returns_expanded)
                q_policy_calibrated = torch.max(q_policy, mc_returns_expanded)
                q_next_policy_calibrated = torch.max(q_next_policy, mc_returns_expanded)
            else:
                q_random_calibrated = q_random
                q_policy_calibrated = q_policy
                q_next_policy_calibrated = q_next_policy
            
            # Importance sampling weights (subtract log probs for proper weighting)
            # For random actions: subtract uniform log prob
            q_random_is = q_random_calibrated / self.cql_temp - random_log_prob
            # For policy actions: subtract policy log prob
            q_policy_is = q_policy_calibrated / self.cql_temp - policy_log_probs / self.cql_temp
            # For next policy actions: subtract next policy log prob  
            q_next_policy_is = q_next_policy_calibrated / self.cql_temp - next_policy_log_probs / self.cql_temp
    ...
    def train(self, logger, current_env_step=None):
        """
        Cal-QL training update for online fine-tuning phase.
        ...
        for i_update in range(num_update):
            ...
            if self.o2o and self.replay_buffer.size > 0:
                obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor, mc_returns_tensor = \
            ...
            # ═══════════════════════════════════════════════════════════════════
            # 1. Q-FUNCTION UPDATE (Cal-QL)
            # ═══════════════════════════════════════════════════════════════════
            total_q_loss, td_loss_val, cql_loss_val, q_mean, cql_diff, alpha_loss, cql_alpha_val = \
                self.compute_calql_q_loss(
                    obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, 
                    done_tensor, mc_returns_tensor, use_calibration=True
                )
        ...
        if num_update > 0 and current_env_step is not None:
            wandb.log({
                "CalQL/td_loss": td_loss_val,
                "CalQL/cql_loss": cql_loss_val,
                "CalQL/policy_loss": policy_loss_val,
                "CalQL/q_mean": q_mean,
                "CalQL/alpha": self.alpha,
                "CalQL/cql_alpha": cql_alpha_val if isinstance(cql_alpha_val, float) else cql_alpha_val.item(),
            }, step=current_env_step)

    def train_offline(self, epochs, test_env=None, current_env_step=None, log_interval=1000, max_ep_len=1000):
        """
        Cal-QL offline pretraining phase.
        
        Uses only offline data with full CQL conservative regularizer + calibration.
        """
        print(f"\n{'='*80}")
        print(f"STARTING CAL-QL OFFLINE TRAINING")
        print(f"{'='*80}")
        ...
        for e in range(epochs):
            ...
            if (e + 1) % log_interval == 0:
                ...
                wandb.log({
                    "CalQL_offline/td_loss": td_loss,
                    "CalQL_offline/cql_loss": cql_loss,
                    "CalQL_offline/policy_loss": policy_loss.item(),
                    "CalQL_offline/q_mean": q_mean,
                    "CalQL_offline/cql_alpha": cql_alpha_log,
                    "CalQL_offline/epoch": e + 1,
                    "offline_training": 1
                }, step=e + 1)
        ...
        print(f"\n{'='*80}")
        print(f"CAL-QL OFFLINE TRAINING COMPLETED")
        print(f"{'='*80}\n")
