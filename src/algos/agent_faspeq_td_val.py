### AGENT CODE FOR FASPEQ TD VAL (TD Error on Validation Set)
### Filename: agent_td_val.py
...
class Agent(object):
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
        ...
        # Validation replay buffer IS used in this version (TD Val)
    ...
    def populate_val_buffer_from_offline(self):
        ...
        print(f"Adding {num_samples} offline samples to validation buffer for TD Eval...")
    ...
    def check_should_trigger_offline_stabilization(self):
        ...
        if self.auto_stab:
            ...
            if current_size >= self.next_trigger_size:
                ...
                print(f"\n{'='*80}\nOFFLINE STABILIZATION TRIGGERED (Adaptive TD-VAL)\n{'='*80}")
                return True
        else:
            if current_size >= self.next_trigger_size:
                self.last_trigger_buffer_size = current_size
                ...
                print(f"\n{'='*80}\nOFFLINE STABILIZATION TRIGGERED (Fixed Interval TD-VAL)\n{'='*80}")
                return True
        return False
    ...
    def evaluate_validation_td_loss(self, batch_size=256):
        """Evaluate TD Error (Q-loss) on Validation Buffer."""
    ...
    def finetune_offline_td_val(self, epochs, test_env=None, current_env_step=None):
        """ 
        FASPEQ (TD-Val Variant): Finetune using TD Error on Validation Set for early stopping.
        Automatic dynamic length enabled by validation check.
        """
        print(f"\n{'='*80}\nSTARTING TD-VAL OFFLINE STABILIZATION\n{'='*80}")
        print(f"Max epochs: {epochs}")
        print(f"Check interval: {self.val_check_interval}")
        print(f"Monitoring: Validation TD Error (MSE)")
        
        initial_loss = self.evaluate_validation_td_loss()
        best_val_loss = initial_loss
        steps_without_improvement = 0
        
        if current_env_step is not None:
            wandb.log({
                "Val_TD_Error": initial_loss,
                "offline_training": 1
            }, step=current_env_step)
        
        print(f"Initial Val TD Error: {initial_loss:.6f}\n")
        ...
        for e in range(epochs):
            ...
            if e > 0 and e % self.val_check_interval == 0:
                val_loss = self.evaluate_validation_td_loss()
                
                if current_env_step is not None:
                    wandb.log({
                        "Val_TD_Error": val_loss,
                        "offline_epoch": e,
                        "offline_training": 1
                    }, step=current_env_step)
                ...
                if val_loss < best_val_loss * 0.999:
                    ...
                    steps_without_improvement = 0
                    print(f"Epoch {e:6d}: Val TD Error improved to {val_loss:.6f}")
                else:
                    steps_without_improvement += self.val_check_interval
                    print(f"Epoch {e:6d}: Val TD Error {val_loss:.6f} (No improve {steps_without_improvement})")
            ...
            # Training on Mixed Data (Online + Offline)
            if self.o2o:
                obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor = self.sample_data_mix(self.batch_size)
            else:
                obs_tensor, obs_next_tensor, acts_tensor, rews_tensor, done_tensor = self.sample_data(self.batch_size)
