...
class FASPEQAgent(SPEQAgent):
    ...
    def __init__(
        ...
        random_early_mean: float = 20000,
        random_early_std: float = 10000,
    ):
        super().__init__(
        ...
        self.random_early = random_early
        self.random_early_mean = random_early_mean
        self.random_early_std = random_early_std
    ...
    def _sample_fixed_validation_batches(self):
        ...
        If no_split=False (default): Sample subset, REMOVE from buffers, validate on held-out
        If no_split=True (ablation): Use ENTIRE training buffer for validation (no removal)
        """
        online_batches = []
        offline_batches = []
        ...
        if self.no_split:
            ...
            if self.replay_buffer.size >= self.batch_size:
                n_complete_batches = self.replay_buffer.size // self.batch_size
                all_indices = np.arange(self.replay_buffer.size)
            ...
            # Offline buffer - use all data
            if self.replay_buffer_offline.size >= self.batch_size:
                n_complete_batches = self.replay_buffer_offline.size // self.batch_size
                all_indices = np.arange(self.replay_buffer_offline.size)
            ...
            print(f"  [NO-SPLIT] Validating on ENTIRE training buffer: {len(online_batches)} online + {len(offline_batches)} offline batches")
            print(f"  Total validation transitions: {total_online} online + {total_offline} offline")
            print(f"  Buffer sizes (unchanged): online={self.replay_buffer.size}, offline={self.replay_buffer_offline.size}")
            return
        
        # DEFAULT BEHAVIOR: sample subset and remove from training
    ...
    def _sample_random_stopping_epoch(self) -> int:
        ...
        # Sample and clip to reasonable range
        sampled = int(np.random.lognormal(mu, sigma))
        sampled = max(1000, min(sampled, self.offline_epochs))
    ...
    def finetune_offline(self, epochs: int = None, test_env=None, current_env_step: int = None) -> int:
        ...
        # For random_early: sample stopping epoch upfront
        if self.random_early:
        ...
        for e in range(epochs):
            ...
            # RANDOM EARLY STOPPING: just check if we hit the random epoch
            if self.random_early:
                if e >= random_stop_epoch:
                    print(f"  [RANDOM-EARLY] Stopping at epoch {e} (random target: {random_stop_epoch})")
                    break
            else:
                # METRIC-BASED EARLY STOPPING (default behavior)
        ...
        # Track stopping epoch for analysis
...
class FASPEQPctAgent(FASPEQAgent):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            no_split=no_split, random_early=random_early,
            random_early_mean=random_early_mean, random_early_std=random_early_std
        )