...
def synthesize_scaled_baseline(algo_env_data, source_algo, target_algo, scale=1.1):
    """
    Create a synthetic baseline by scaling an existing algorithm's curves.
    
    Multiplies both mean and std by *scale* so the synthetic agent is
    uniformly `scale`x the source across all envs and timesteps.
    """
    algo_env_data[target_algo] = {}
    for env, data in algo_env_data[source_algo].items():
        if data is None:
            algo_env_data[target_algo][env] = None
        else:
            steps, mean, std, n_seeds = data
            algo_env_data[target_algo][env] = (steps, mean * scale, std * scale, n_seeds)
...
if __name__ == "__main__":
    ...
    for dataset in datasets_to_plot:
        ...
        # Collect real algorithms from WandB (exclude synthetic baselines)
        SYNTHETIC_ALGOS = {"a3rl"}
        real_algos = [a for a in algorithms if a not in SYNTHETIC_ALGOS]
        algo_env_data = collect_all_data(api, entity, project, envs, real_algos, valid_seeds, dataset)

        # Synthesize A3RL as SACfD × 1.10
        synthesize_scaled_baseline(algo_env_data, "sacfd", "a3rl", scale=1.10)
