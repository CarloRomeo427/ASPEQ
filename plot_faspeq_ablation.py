...
def auto_label(algo):
    ...
    # Handle patience ablations: faspeq_pct10_pi_valpat{N}
    if algo.startswith('faspeq_pct10_pi_valpat'):
        patience = algo.replace('faspeq_pct10_pi_valpat', '')
        return f"FASPEQ 10% (π) pat={patience}"
...
def get_env_variants(env):
    ...
    variants = [env]
    # Also try with different capitalizations
    ...
    # Handle Halfcheetah vs HalfCheetah
...
def build_run_name_variants(algo, env, dataset):
    ...
    Naming conventions:
    - sacfd: sacfd_{Env}_{dataset} (e.g., sacfd_Halfcheetah-v5_medium)
    - faspeq_pct10_pi: faspeq_pct10_pi_{Env}_{dataset} (e.g., faspeq_pct10_pi_Halfcheetah-v5_medium)
    - faspeq_pct10_pi_valpat{N}: faspeq_pct10_pi_{Env}_{dataset}_valpat{N}
    """
    variants = []
    
    for env_variant in get_env_variants(env):
        if algo.startswith('faspeq_pct10_pi_valpat'):
            # Ablation: faspeq_pct10_pi_{env}_{dataset}_valpat{N}
            patience = algo.replace('faspeq_pct10_pi_valpat', '')
            run_name = f"faspeq_pct10_pi_{env_variant}_{dataset}_valpat{patience}"
        elif algo == 'faspeq_pct10_pi':
            # Base FASPEQ: faspeq_pct10_pi_{env}_{dataset}
            run_name = f"faspeq_pct10_pi_{env_variant}_{dataset}"
        elif algo == 'sacfd':
            # SACfD: sacfd_{Env}_{dataset}
            run_name = f"sacfd_{env_variant}_{dataset}"
        else:
...
def plot_combined_envs(algo_env_data, envs, algorithms, dataset, ema_alpha, std_alpha):
    ...
    plt.suptitle(f"FASPEQ Patience Ablation - {dataset.upper()} Dataset", fontsize=16, fontweight='bold')
    plt.tight_layout()
    return fig
...
if __name__ == "__main__":
    ...
    # - sacfd (baseline)
    # - faspeq_pct10_pi (default patience=2000)
    # - faspeq_pct10_pi_valpat{1000,3000,5000} (ablations)
    # ============================================================
    algorithms = [
        "sacfd",
        "faspeq_pct10_pi",
    ...
    print("="*80)
    print("FASPEQ PATIENCE ABLATION STUDY")
    print("="*80)
    ...
    for dataset in datasets_to_plot:
        ...
        if args.individual:
            for env in envs:
                ...
                plt.tight_layout()
                plt.savefig(f"Plots/ablation_{env}_{dataset}.png", dpi=300, bbox_inches="tight")
                plt.close()
            ...
            plt.savefig(f"Plots/ablation_combined_{dataset}.png", dpi=300, bbox_inches="tight")
            print(f"\nSaved: Plots/ablation_combined_{dataset}.png")
            plt.close()
        ...
        if not args.no_aggregated:
            ...
            plt.tight_layout()
            plt.savefig(f"Plots/ablation_aggregated_{dataset}.png", dpi=300, bbox_inches="tight")
            print(f"Saved: Plots/ablation_aggregated_{dataset}.png")
            plt.close()
    ...
    if len(datasets_to_plot) == 3 and not args.no_aggregated:
        ...
        ax.set_title("FASPEQ Patience Ablation - ALL DATASETS (Expert + Medium + Simple)", fontsize=14, fontweight='bold')
        ax.set_xlabel("Steps", fontsize=12)
        ...
        plt.tight_layout()
        plt.savefig("Plots/ablation_aggregated_all_datasets.png", dpi=300, bbox_inches="tight")
        print(f"\nSaved: Plots/ablation_aggregated_all_datasets.png")
        plt.close()
